...
from tgi_profiler.tgi_container import TGIContainer


@pytest.fixture
def mock_docker():
    """Fixture providing a mocked Docker client"""
    with patch('tgi_profiler.tgi_container.docker') as mock_docker:
...
def test_token_calculations(single_gpu_config):
    ...
    assert total_tokens == (single_gpu_config.max_input_length +
                            single_gpu_config.max_output_length)
